...
// Уровни «течи» по баллу — стикер реальной Ульяны с нужной силой плача
function level(score: number): { name: string; sticker: number } {
  if (score < 15) return { name: 'Сухой глаз', sticker: UL.calm };
  if (score < 35) return { name: 'Лёгкая течь', sticker: UL.almost };
  if (score < 55) return { name: 'Капель', sticker: UL.cry1 };
  if (score < 75) return { name: 'Прорыв плотины', sticker: UL.cry2 };
  if (score < 90) return { name: 'Шторм 9 баллов', sticker: UL.sob };
  return { name: 'Севастопольский потоп', sticker: UL.flood };
}
...
}): { score: number; verdict: string; recommendation: string; levelName: string; sticker: number } {
