...
def main() -> int:
    ...
    with sync_playwright() as playwright:
        ...
        page.get_by_label('Show navigation menu').last.click()
        page.wait_for_timeout(600)
        page.get_by_text('Рабочее место', exact=True).wait_for(timeout=5_000)
        page.wait_for_timeout(500)
        assert page.get_by_text('Компьютер', exact=True).first.is_visible()
        assert page.get_by_text('Помощник', exact=True).first.is_visible()
        assert page.get_by_text('Настройки', exact=True).first.is_visible()
        assert page.get_by_text('Проекты', exact=True).count() == 0
        page.screenshot(path=str(DRAWER_OUTPUT), full_page=True)
        page.get_by_text('Компьютер', exact=True).first.click()
        page.get_by_text('Nodex', exact=True).wait_for(timeout=8_000)
        page.get_by_text('Nodex', exact=True).click()
        page.get_by_placeholder('Найти проект').wait_for(timeout=8_000)
        page.get_by_text('Продолжить редизайн Noda', exact=True).wait_for(timeout=5_000)
        page.get_by_text('Продолжить редизайн Noda', exact=True).click()
        page.get_by_text('Что делаем в Noda?', exact=True).last.wait_for(timeout=5_000)
        ...
        page.wait_for_timeout(500)
        page.get_by_text('Осталось вернуть', exact=True).wait_for(timeout=5_000)
